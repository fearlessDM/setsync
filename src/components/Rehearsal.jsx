    ...
    <div style={{position:'fixed',inset:0,zIndex:100,background:'var(--bg)',display:'flex',flexDirection:'column'}}>
      ...
      <div className="sv-hdr">
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontFamily:"'Lexend Giga',sans-serif",fontWeight:900,fontSize:'var(--fs-xl)',color:'var(--tx)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{song.name}</div>
        ...
        <button onClick={onClose} style={{width:28,height:28,borderRadius:8,border:'1px solid var(--bd)',background:'var(--s1)',color:'var(--tx3)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'var(--fs-xl)',flexShrink:0}}>×</button>
      </div>
        ...
        {showAnnoBar&&(
          <div style={{display:'flex',alignItems:'center',gap:3,padding:'0 10px 5px',overflowX:'auto',scrollbarWidth:'none'}}>
            {[['draw','Lápiz'],['erase','Borrar']].map(([t,l])=>(
              <button key={t} onClick={()=>setTool(t)} style={{display:'flex',alignItems:'center',gap:4,padding:'3px 8px',borderRadius:6,border:tool===t?'1px solid var(--bd)':'1px solid transparent',background:tool===t?'var(--s3)':'transparent',color:tool===t?'var(--tx)':'var(--tx3)',cursor:'pointer',fontSize:'var(--fs-base)',fontWeight:700,fontFamily:"'Lexend Giga',sans-serif",flexShrink:0}}>{l}</button>
            ))}
      ...
      <div style={{flex:1,position:'relative',overflow:'hidden'}}>
        ...
        <div style={{position:'absolute',right:6,top:'50%',transform:'translateY(-50%)',zIndex:3,display:'flex',flexDirection:'column',alignItems:'center',gap:0,borderRadius:14,border:'1px solid var(--bd)',background:'rgba(8,8,20,.88)',backdropFilter:'blur(12px)',overflow:'hidden',width:50}}>
          ...
          <div style={{width:'100%',padding:'8px 0',textAlign:'center',borderBottom:'1px solid var(--bd)'}}>
            <div style={{fontFamily:"'Lexend Giga',sans-serif",fontWeight:900,fontSize:'var(--fs-2xl)',color:'var(--ac)',lineHeight:1}}>{curKey}</div>
          ...
          <div style={{borderTop:'1px solid var(--bd)',width:'100%',display:'flex',flexDirection:'column',alignItems:'center',paddingTop:4}}>
            ...
            {[0,1,2,3,4,5,6].map(c=>(
              <button key={c} onClick={()=>setCapoR(c)} style={{width:38,height:22,border:'none',background:capoR===c?'rgba(200,169,126,.2)':'transparent',color:capoR===c?'var(--ac)':'var(--tx3)',fontSize:'var(--fs-sm)',fontWeight:capoR===c?900:400,cursor:'pointer',fontFamily:"'Lexend Giga',sans-serif",borderRadius:5,transition:'all .1s'}}>
                {c===0?'—':c}
              </button>
            ))}
          </div>
      ...
      <div className="sv-nav">
        ...
        <div className="sv-ni">
          <div style={{fontFamily:"'Lexend Giga',sans-serif",fontWeight:900,fontSize:'var(--fs-lg)',color:'var(--tx)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{song.name}</div>
