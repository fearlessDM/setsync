  ...
  return(
    <div style={{padding:'0 0 90px'}}>
      ...
      {eventos.filter(ev=>{
        ...
        return d.getMonth()===mesNav;
      }).length>0&&(
        <div style={{marginTop:8,padding:'0 8px',display:'flex',flexDirection:'column',gap:10}}>
          <div style={{display:'flex',alignItems:'center',gap:8,padding:'4px 0'}}>
            <div style={{flex:1,height:1,background:'rgba(255,255,255,.08)'}}/>
            <span style={{fontSize:9,fontWeight:900,color:'var(--tx3)',textTransform:'uppercase',letterSpacing:'1.5px'}}>Eventos creados</span>
            <div style={{flex:1,height:1,background:'rgba(255,255,255,.08)'}}/>
          </div>
          {eventos.filter(ev=>ev.fecha&&new Date(ev.fecha).getMonth()===mesNav)
