...
export function MiniCalEvento({mes,eventos=[],onSelectDay=()=>{},selectedDay=null}){
  ...
  const eventosDays=new Set(
    eventos.filter(e=>e.fecha&&new Date(e.fecha).getFullYear()===year&&new Date(e.fecha).getMonth()===month)
...
function BarraMeses({mesActivo, onChange}){
  const scrollRef=useRef(null);
  // Meses con eventos disponibles (basado en EVENTOS_ESPECIALES)
  const mesesConEventos=new Set(EVENTOS_ESPECIALES.map(e=>e.mes-1));
  // Julio (6) siempre tiene datos demo (SETLISTS)
  mesesConEventos.add(6);

  // Scroll al mes activo al montar
  useEffect(()=>{
  ...
  return(
    <div ref={scrollRef} style={{
      ...
      padding:'0 var(--pw-x,16px) 0',
      scrollbarWidth:'none',
      WebkitOverflowScrolling:'touch',
    }}>
      {MESES_CORTO.map((m,i)=>{
        const isActive=i===mesActivo;
        const hasDot=mesesConEventos.has(i);
        return(
          <button
            key={i}
            data-mes={i}
          ...
          >
            {m}
            {hasDot&&!isActive&&(
              <div style={{width:4,height:4,borderRadius:'50%',background:'var(--gn)',flexShrink:0}}/>
            )}
          </button>
        );
      })}
...
export function AdminView({mode,activeSunday,userRole,onLive,onToast,onSelectDay,mesNav=new Date().getMonth(),lang='es',eventos=[],onOpenSong,equipos=[]}){
  ...
  // Eventos de Firestore para este mes
  const eventosDelMes=eventos.filter(ev=>{
    if(!ev.fecha) return false;
    return new Date(ev.fecha).getMonth()===mesNav;
  }).sort((a,b)=>new Date(a.fecha)-new Date(b.fecha));

  // Eventos especiales de este mes (de constants)
  const especialesDelMes=EVENTOS_ESPECIALES.filter(ev=>ev.mes===mesNav+1);

  // Setlists de este mes (julio = índice 6)
  ...
  return(
    <div style={{paddingBottom:90}}>
      ...
      {/* ── Barra de meses ── */}
      <div style={{paddingTop:'var(--sp-sm)',paddingBottom:'var(--sp-xs)'}}>
        <BarraMeses mesActivo={mesNav} onChange={handleMesChange}/>
      </div>

      {/* ── Header: mes + navegación + mini cal ── */}
      <div style={{padding:'var(--sp-xs) var(--pw-x,16px) var(--sp-sm)',display:'flex',alignItems:'flex-start',gap:'var(--sp-sm)'}}>
        <div style={{flex:1}}>
          <div style={{display:'flex',alignItems:'center',gap:'var(--sp-xs)',marginBottom:4}}>
            <button
              onClick={()=>handleMesChange(Math.max(0,mesNav-1))}
              style={{width:28,height:28,borderRadius:'var(--rad-xs)',border:'1px solid var(--bd)',background:'transparent',color:'var(--tx3)',cursor:mesNav===0?'not-allowed':'pointer',display:'flex',alignItems:'center',justifyContent:'center',opacity:mesNav===0?.35:1}}
              disabled={mesNav===0}
            >
            ...
            <button
              ...
              style={{width:28,height:28,borderRadius:'var(--rad-xs)',border:'1px solid var(--bd)',background:'transparent',color:'var(--tx3)',cursor:mesNav===11?'not-allowed':'pointer',display:'flex',alignItems:'center',justifyContent:'center',opacity:mesNav===11?.35:1}}
              disabled={mesNav===11}
            >
          ...
          <div style={{fontFamily:"'Lexend Giga',sans-serif",fontWeight:300,fontSize:11,color:'var(--tx3)',lineHeight:1.5}}>
            {setlistsDelMes.length>0||especialesDelMes.length>0||eventosDelMes.length>0
              ? 'Toca un bloque para ver el setlist'
      ...
      {/* ── Bloques de setlists (julio) ── */}
      {setlistsDelMes.length>0&&(
        <div style={{display:'flex',flexDirection:'column',gap:'var(--gap)',padding:'0 var(--pw-x,16px) var(--sp-md)'}}>
          {setlistsDelMes.map(([dayStr,sl])=>{
            ...
            const pub=day<=14;
            if(isNull)return(
              <div key={day} style={{padding:'var(--sp-md)',borderRadius:'var(--rad-md)',background:'rgba(255,82,82,.04)',border:'1px solid rgba(255,82,82,.15)',display:'flex',alignItems:'center',gap:'var(--sp-sm)'}}>
            ...
            return(
              <div key={day}>
                {isNext&&day>today&&(
                  <div style={{display:'flex',alignItems:'center',gap:'var(--sp-xs)',margin:'4px 0 var(--sp-xs)'}}>
                ...
                <div
                  onClick={()=>{setSelDay(day);if(onSelectDay)onSelectDay(day);if(sl&&sl.length>0&&onOpenSong)onOpenSong(0,sl);}}
                  style={{
                    padding:'var(--sp-md)',borderRadius:'var(--rad-md)',
                    background:isNext?'rgba(255,255,255,.04)':'var(--s1)',
                    border:isNext?'1px solid rgba(255,255,255,.15)':'1px solid var(--bd)',
                    cursor:'pointer',transition:'all .2s',
                    opacity:(day<today&&!isActive)?.55:1,
                  }}
                >
                  <div style={{display:'flex',alignItems:'center',gap:'var(--sp-sm)',marginBottom:'var(--sp-sm)'}}>
                    <div style={{flex:1}}>
                      <div style={{fontFamily:"'Special Gothic Expanded One',sans-serif",fontWeight:400,color:'var(--tx)',lineHeight:1.1,fontSize:18}}>
                        {tx.sunday} {day}
                      </div>
                      <div style={{fontFamily:"'Lexend Giga',sans-serif",fontWeight:300,fontSize:11,color:'var(--tx3)',lineHeight:1.5,marginTop:4}}>
                        {sl.length} {sl.length===1?tx.song:tx.songs}
                      </div>
                    </div>
                    <span style={{padding:'4px 10px',borderRadius:'var(--rad-full)',fontSize:9,fontWeight:700,border:pub?'1px solid rgba(94,206,160,.35)':'1px solid rgba(255,200,100,.25)',background:pub?'rgba(94,206,160,.08)':'rgba(255,200,100,.06)',color:pub?'var(--gn)':'rgba(255,200,100,.8)',flexShrink:0}}>
                      {pub?tx.published:tx.draft}
                    </span>
                    {isLeader&&sl&&sl.length>0&&(
                      <button
                        onClick={e=>{e.stopPropagation();if(onOpenSong)onOpenSong(0,sl);}}
                        style={{padding:'7px 12px',borderRadius:'var(--rad-sm)',cursor:'pointer',display:'flex',alignItems:'center',gap:5,flexShrink:0,fontFamily:"'Lexend Giga',sans-serif",fontWeight:700,fontSize:11,color:'var(--gn)',border:'1px solid rgba(94,206,160,.35)',background:'rgba(94,206,160,.1)'}}
                      >
                  ...
                  {equipos.length>0&&(
                    <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:2}}>
                      {equipos.map(eq=>(
                        <div key={eq.id} style={{display:'flex',alignItems:'center',gap:5,padding:'3px 10px',borderRadius:'var(--rad-full)',background:eq.color+'12',border:'1px solid '+eq.color+'30'}}>
      ...
      {eventosDelMes.length>0&&(
        <div style={{padding:'0 var(--pw-x,16px) var(--sp-md)'}}>
          <div style={{display:'flex',alignItems:'center',gap:'var(--sp-xs)',padding:'4px 0',marginBottom:'var(--gap)'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'var(--gap)'}}>
            {eventosDelMes.map(ev=>{
              ...
              return(
                <div key={ev.id}
                  style={{padding:'var(--sp-md)',borderRadius:'var(--rad-md)',background:'var(--s1)',border:'1px solid var(--bd)',cursor:onOpenSong&&(ev.setlist||[]).length>0?'pointer':'default'}}
                  onClick={()=>onOpenSong&&ev.setlist&&(ev.setlist||[]).length>0&&onOpenSong(0,ev.setlist)}
                >
                  <div style={{display:'flex',alignItems:'center',gap:'var(--sp-sm)'}}>
                    <div style={{width:38,textAlign:'center',flexShrink:0,background:'var(--s2)',borderRadius:'var(--rad-xs)',padding:'6px 4px'}}>
                      ...
                      <div style={{fontSize:8,fontWeight:700,color:'var(--tx3)',textTransform:'uppercase',marginTop:2,fontFamily:"'Lexend Giga',sans-serif"}}>{MESES_CORTO[d.getMonth()]}</div>
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontFamily:"'Special Gothic Expanded One',sans-serif",fontWeight:400,fontSize:15,color:'var(--tx)'}}>{ev.nombre}</div>
                      <div style={{fontSize:11,color:'var(--tx3)',marginTop:3}}>
                        {ev.lugar?`${ev.lugar} · `:''}{(ev.setlist||[]).length} canciones
                      </div>
                    </div>
                    {isLeader&&(ev.setlist||[]).length>0&&(
      ...
      {/* ── Eventos especiales (de constants) ── */}
        ...
        <div style={{padding:'0 var(--pw-x,16px) var(--sp-md)'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'var(--gap)'}}>
            {especialesDelMes.map((ev,i)=>(
              <div key={i}
                style={{padding:'var(--sp-md)',borderRadius:'var(--rad-md)',background:'var(--s1)',border:'1px solid rgba(200,169,126,.25)',cursor:'pointer'}}
                onClick={()=>{if(onSelectDay)onSelectDay(ev.dia);if(onOpenSong&&(ev.setlist||[]).length>0)onOpenSong(0,ev.setlist);}}
              >
                <div style={{display:'flex',alignItems:'center',gap:'var(--sp-sm)',marginBottom:ev.setlist?.length>0?'var(--sp-xs)':0}}>
                  <div style={{flex:1}}>
                    <div style={{fontFamily:"'Special Gothic Expanded One',sans-serif",fontWeight:400,color:'var(--tx)',lineHeight:1.1,fontSize:17}}>{ev.label}</div>
                    <div style={{fontFamily:"'Lexend Giga',sans-serif",fontWeight:300,fontSize:11,color:'var(--tx3)',lineHeight:1.5,marginTop:3,textTransform:'capitalize'}}>
                      {ev.tipo} · {(ev.setlist||[]).length} canciones
                    </div>
                  </div>
                  {isLeader&&(ev.setlist||[]).length>0&&(
                    <button
                      onClick={e=>{e.stopPropagation();onLive&&onLive();}}
                      style={{padding:'6px 10px',borderRadius:'var(--rad-sm)',border:'1px solid rgba(200,169,126,.3)',background:'rgba(200,169,126,.08)',cursor:'pointer',fontSize:10,fontWeight:700,color:'var(--ac)',fontFamily:"'Lexend Giga',sans-serif",display:'flex',alignItems:'center',gap:4,flexShrink:0}}
                    >
                ...
                {(ev.setlist||[]).length>0&&(
                  <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
                    {ev.setlist.map((s,j)=>(
                      <span key={j} style={{fontSize:9,fontWeight:700,color:'var(--tx3)',background:'rgba(255,255,255,.06)',border:'1px solid rgba(255,255,255,.07)',padding:'2px 7px',borderRadius:'var(--rad-full)'}}>
      ...
      {setlistsDelMes.length===0&&especialesDelMes.length===0&&eventosDelMes.length===0&&(
        <div style={{padding:'var(--sp-xl) var(--pw-x,16px)'}}>
          <div style={{padding:'32px 20px',borderRadius:'var(--rad-lg)',background:'var(--s1)',border:'1px solid var(--bd)',textAlign:'center'}}>
            <div style={{fontSize:32,marginBottom:'var(--sp-sm)'}}>📭</div>
            <div style={{fontFamily:"'Special Gothic Expanded One',sans-serif",fontWeight:400,fontSize:17,color:'var(--tx)',marginBottom:6}}>{tx.noEvents||'Sin eventos'}</div>
            <div style={{fontSize:12,color:'var(--ac)',fontWeight:600}}>{tx.noEventsSub||'Crea un evento desde Backstage'}</div>
          </div>
        </div>
      )}

      {/* ── Modal picker (agregar canción) ── */}
      {showPicker&&(
    ...
    <div style={{marginBottom:'var(--sp-md)',padding:'var(--sp-md)',borderRadius:'var(--rad-md)',border:'1px solid rgba(200,169,126,.28)',background:'rgba(200,169,126,.05)'}}>
      ...
      <div style={{display:'flex',gap:'var(--sp-xs)',marginBottom:'var(--sp-sm)'}}>
        {[['mie','Miércoles'],['sab','Sábado']].map(([t,l])=>(
          <button key={t} onClick={()=>setTipo(t)} style={{flex:1,padding:'8px',borderRadius:'var(--rad-sm)',border:tipo===t?'1px solid rgba(200,169,126,.4)':'1px solid var(--bd)',background:tipo===t?'rgba(200,169,126,.08)':'var(--s1)',color:tipo===t?'var(--ac)':'var(--tx3)',fontWeight:700,fontSize:12,cursor:'pointer',fontFamily:"'Lexend Giga',sans-serif"}}>
            {l}
          </button>
        ))}
      </div>
    ...
    <div style={{padding:'var(--sp-sm) var(--pw-x,16px)',paddingBottom:90}}>
      ...
      <div style={{background:'var(--s1)',border:'1px solid var(--bd)',borderRadius:'var(--rad-md)',marginBottom:'var(--gap)',overflow:'hidden'}}>
        ...
        {sl.length===0
          ...
          :sl.map((s,i)=>(
            <div key={i} onClick={()=>onOpenSong(i)} style={{display:'flex',alignItems:'center',gap:'var(--sp-sm)',padding:'12px var(--sp-md)',borderBottom:i<sl.length-1?'1px solid rgba(255,255,255,.05)':'none',cursor:'pointer'}}>
      ...
      <div style={{background:'var(--s1)',border:'1px solid var(--bd)',borderRadius:'var(--rad-md)',overflow:'hidden'}}>
        ...
        {ITINERARIO.map((it,i)=>(
          <div key={i} style={{display:'flex',gap:'var(--sp-sm)',padding:'10px var(--sp-md)',borderBottom:i<ITINERARIO.length-1?'1px solid rgba(255,255,255,.04)':'none',alignItems:'flex-start'}}>
...
export function PremiereView({onToast}){
  ...
  if(sel){
    ...
    return(
      <div style={{padding:'0 0 90px'}}>
        <div style={{padding:'var(--sp-md)',display:'flex',alignItems:'center',gap:10,marginBottom:4,cursor:'pointer'}} onClick={()=>setSel(null)}>
        ...
        <div style={{margin:'0 var(--pw-x,16px)',padding:'var(--sp-lg)',borderRadius:'var(--rad-md)',background:'linear-gradient(135deg,rgba(200,169,126,.15),rgba(100,80,180,.1))',border:'1px solid rgba(200,169,126,.3)',marginBottom:'var(--sp-md)'}}>
          <div style={{fontSize:10,fontWeight:900,color:'var(--ac)',textTransform:'uppercase',letterSpacing:'2px',marginBottom:'var(--sp-xs)'}}>{p.oficial?'✓ Cifrado oficial':'Próximamente'}</div>
          ...
          <div style={{fontSize:13,color:'var(--tx2)',marginBottom:'var(--sp-sm)'}}>{p.album&&`${p.album} · `}{p.sello}</div>
          <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:'var(--sp-md)'}}>
            <span style={{fontSize:11,fontWeight:700,color:'var(--ac)',background:'rgba(200,169,126,.1)',border:'1px solid rgba(200,169,126,.25)',padding:'4px 10px',borderRadius:'var(--rad-full)'}}>{p.key}</span>
            <span style={{fontSize:11,fontWeight:700,color:'var(--tx2)',background:'var(--s1)',border:'1px solid var(--bd)',padding:'4px 10px',borderRadius:'var(--rad-full)'}}>{p.bpm} BPM</span>
            {p.dias&&<span style={{fontSize:11,fontWeight:700,color:'var(--rd)',background:'rgba(255,82,82,.1)',border:'1px solid rgba(255,82,82,.28)',padding:'4px 10px',borderRadius:'var(--rad-full)'}}>⚡ En {p.dias} días</span>}
          </div>
          <div style={{fontSize:13,color:'var(--tx2)',lineHeight:1.7}}>{p.desc}</div>
        </div>
        <div style={{padding:'0 var(--pw-x,16px)',display:'flex',flexDirection:'column',gap:'var(--sp-xs)'}}>
  ...
  return(
    <div style={{padding:'0 0 90px'}}>
      <div style={{margin:'var(--sp-md) var(--pw-x,16px)',borderRadius:'var(--rad-lg)',overflow:'hidden',border:'1px solid rgba(200,169,126,.25)',cursor:'pointer'}} onClick={()=>setSel(top.id)}>
        <div style={{background:'linear-gradient(135deg,rgba(10,8,20,.95),rgba(30,20,60,.92))',padding:'22px 20px 20px'}}>
          ...
          <div style={{fontSize:12,color:'var(--tx2)',marginBottom:'var(--sp-md)'}}>{top.album} · {top.sello}</div>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <span style={{fontSize:12,fontWeight:700,color:'var(--rd)',background:'rgba(255,82,82,.12)',border:'1px solid rgba(255,82,82,.3)',padding:'5px 12px',borderRadius:'var(--rad-full)'}}>⚡ En {top.dias} días</span>
            <span style={{fontSize:11,color:'var(--tx3)',fontWeight:700}}>🏛 247 iglesias →</span>
          </div>
        </div>
      </div>
      <div style={{padding:'0 var(--pw-x,16px)',display:'flex',flexDirection:'column',gap:'var(--sp-xs)'}}>
        {PREMIERES.map(p=>(
          <div key={p.id} onClick={()=>setSel(p.id)} style={{padding:'var(--sp-md)',borderRadius:'var(--rad-md)',background:'var(--s1)',border:'1px solid var(--bd)',cursor:'pointer',display:'flex',alignItems:'center',gap:'var(--sp-sm)'}}>
            <div style={{flex:1}}>
      ...
      <div style={{padding:'var(--sp-md)',marginTop:'var(--sp-xs)',borderTop:'1px solid var(--bd)'}}>
        <div style={{fontSize:11,color:'var(--tx3)',textAlign:'center',lineHeight:1.7}}>
          ¿Representas un sello o artista?<br/>
